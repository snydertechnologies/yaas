...
export default class SaleReceiptInventoryTransactionsSubscriber {
   ...
   */
  public attach(bus) {
    bus.subscribe(events.saleReceipt.onCreated, this.handleWritingInventoryTransactions);
    bus.subscribe(events.saleReceipt.onEdited, this.handleRewritingInventoryTransactions);
    bus.subscribe(events.saleReceipt.onDeleted, this.handleDeletingInventoryTransactions);
  }
   ...
   */
  private handleWritingInventoryTransactions = async ({ tenantId, saleReceipt, trx }: ISaleReceiptCreatedPayload) => {
    // Can't continue if the sale receipt is not closed yet.
    if (!saleReceipt.closedAt) return null;

    await this.saleReceiptInventory.recordInventoryTransactions(tenantId, saleReceipt, false, trx);
  };
   ...
   */
  private handleRewritingInventoryTransactions = async ({ tenantId, saleReceipt, trx }: ISaleReceiptEditedPayload) => {
    // Can't continue if the sale receipt is not closed yet.
    if (!saleReceipt.closedAt) return null;

    await this.saleReceiptInventory.recordInventoryTransactions(tenantId, saleReceipt, true, trx);
  };
  ...
  private handleDeletingInventoryTransactions = async ({
    ...
    await this.saleReceiptInventory.revertInventoryTransactions(tenantId, saleReceiptId, trx);
  };
}
