import { ISaleEstimate } from '@bigcapital/libs-backend';
import { ServiceError } from '@bigcapital/server/exceptions';
import { SaleEstimate } from '@bigcapital/server/models';
...
export class SaleEstimateValidators {
  ...
  public async validateEstimateNumberExistance(tenantId: number, estimateNumber: string, notEstimateId?: number) {
    ...
    if (foundSaleEstimate) {
      throw new ServiceError(ERRORS.SALE_ESTIMATE_NUMBER_EXISTANCE, 'The given sale estimate is not unique.');
    }
  }
