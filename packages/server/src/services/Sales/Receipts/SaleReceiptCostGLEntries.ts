import { AccountNormal, IInventoryLotCost, ILedgerEntry } from '@bigcapital/server/interfaces';
...
export class SaleReceiptCostGLEntries {
  ...
  private getInventoryCostLotsLedger = (inventoryCostLots: IInventoryLotCost[]) => {
    ...
    //
    const entries = inventoryTransactions.map(this.getSaleInvoiceCostGLEntries).flat();
