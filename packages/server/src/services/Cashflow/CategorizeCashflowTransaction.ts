...
export class CategorizeCashflowTransaction {
  ...
  public async categorize(
    ...
    this.commandValidators.validateUncategorizeTransactionType(transaction, categorizeDTO.transactionType);
    ...
    return this.uow.withTransaction(tenantId, async (trx: Knex.Transaction) => {
      ...
      const cashflowTransactionDTO = transformCategorizeTransToCashflow(transaction, categorizeDTO);
      // Creates a new cashflow transaction.
      const cashflowTransaction = await this.createCashflow.newCashflowTransaction(tenantId, cashflowTransactionDTO);
      // Updates the uncategorized transaction as categorized.
      await UncategorizedCashflowTransaction.query(trx).patchAndFetchById(uncategorizedTransactionId, {
        categorized: true,
