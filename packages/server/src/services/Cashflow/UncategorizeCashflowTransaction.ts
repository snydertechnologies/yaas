...
export class UncategorizeCashflowTransaction {
   ...
   */
  public async uncategorize(tenantId: number, uncategorizedTransactionId: number) {
    const { UncategorizedCashflowTransaction } = this.tenancy.models(tenantId);

    const oldUncategorizedTransaction = await UncategorizedCashflowTransaction.query()
    ...
    return this.uow.withTransaction(tenantId, async (trx: Knex.Transaction) => {
      ...
      await this.eventPublisher.emitAsync(events.cashflow.onTransactionUncategorizing, {
        tenantId,
        trx,
      } as ICashflowTransactionUncategorizingPayload);
      // Removes the ref relation with the related transaction.
      const uncategorizedTransaction = await UncategorizedCashflowTransaction.query(trx).updateAndFetchById(
      ...
      await this.eventPublisher.emitAsync(events.cashflow.onTransactionUncategorized, {
        tenantId,
