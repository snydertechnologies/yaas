...
export default class ARAgingSummarySheet extends AgingSummaryReport {
   ...
   */
  private customerTransformer = (customer: ICustomer): IARAgingSummaryCustomer => {
   ...
   */
  private customersMapper = (customers: ICustomer[]): IARAgingSummaryCustomer[] => {
   ...
   * @returns {boolean}
   */
  private filterNoneZeroAccountNode = (node: IARAgingSummaryCustomer): boolean => {
   ...
   * @returns {boolean}
   */
  private customerNodeFilter = (customerNode: IARAgingSummaryCustomer): boolean => {
   ...
   * @returns {IARAgingSummaryCustomer[]}
   */
  private customersFilter = (customers: IARAgingSummaryCustomer[]): IARAgingSummaryCustomer[] => {
  ...
  private customersWalker = (customers: ICustomer[]): IARAgingSummaryCustomer[] => {
    return R.compose(R.when(this.isCustomersFilterEnabled, this.customersFilter), this.customersMapper)(customers);
  };
   ...
   */
  private getCustomersTotal = (customersAgingPeriods: IARAgingSummaryCustomer[]): IARAgingSummaryTotal => {
