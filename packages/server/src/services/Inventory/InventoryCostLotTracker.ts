...
export default class InventoryCostLotTracker extends InventoryCostMethod implements IInventoryCostMethod {
   ...
   */
  constructor(tenantId: number, startingDate: Date, itemId: number, costMethod: TCostMethod = 'FIFO') {
  ...
  public async computeItemCost(): Promise<any> {
    ...
    const storedTrackedInvLotsOper = this.storeInventoryLotsCost(this.costLotsTransactions);
    return Promise.all([storedTrackedInvLotsOper]);
  }
  ...
  private async fetchInvINTransactions() {
    ...
    const availableINLots: IInventoryLotCost[] = await InventoryLotCostTracker.query()
  ...
  private async fetchInvOUTTransactions() {
    ...
    const afterOUTTransactions: IInventoryTransaction[] = await InventoryTransaction.query()
      .modify('filterDateRange', this.startingDate)
      .orderBy('date', 'ASC')
      .orderBy('lot_number', 'ASC')
  ...
  private async fetchItemsMapped() {
    const itemsIds = chain(this.inTransactions)
      .map((e) => e.itemId)
      .uniq()
      .value();
    const { Item } = this.tenantModels;
    const storedItems = await Item.query().where('type', 'inventory').whereIn('id', itemsIds);
  ...
  private async fetchRevertInvJReferenceIds() {
    const { InventoryTransaction } = this.tenantModels;
    const revertJEntriesTransactions: IInventoryTransaction[] = await InventoryTransaction.query()
   ...
   * @return {void}
   */
  public trackingInventoryINLots(inventoryTransactions: IInventoryTransaction[]) {
   ...
   * @return {void}
   */
  public trackingInventoryOUTLots(inventoryTransactions: IInventoryTransaction[]) {
    inventoryTransactions.forEach((transaction: IInventoryTransaction) => {
      const { itemId, id } = transaction;
      this.inventoryByItem[itemId] || (this.inventoryByItem[itemId] = []);
      ...
      this.inventoryByItem?.[itemId]?.some((_invTransactionId: number) => {
        ...
        _invINTransaction.remaining = Math.max(_invINTransaction.remaining - maxDecrement, 0);
  ...
  private removeInventoryItems(itemId: number, idsShouldDel: number[]) {
    ...
    this.inventoryByItem[itemId] = this.inventoryByItem?.[itemId]?.filter(
      (transId: number) => idsShouldDel.indexOf(transId) === -1,
    );
  }
}
