import { ITaxRateActivatedPayload, ITaxRateActivatingPayload } from '@bigcapital/libs-backend';
...
export class ActivateTaxRateService {
  ...
  public activateTaxRate(tenantId: number, taxRateId: number) {
    ...
    return this.uow.withTransaction(tenantId, async (trx: Knex.Transaction) => {
      ...
      const taxRate = await TaxRate.query(trx).findById(taxRateId).patch({ active: 1 });
