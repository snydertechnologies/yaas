...
import { IVendorCreditApplyToBillDeletedPayload } from '@bigcapital/server/interfaces';
...
export default class DeleteApplyVendorCreditToBill extends BaseVendorCredit {
   ...
   */
  public deleteApplyVendorCreditToBills = async (tenantId: number, appliedCreditToBillId: number) => {
    const { VendorCreditAppliedBill } = this.tenancy.models(tenantId);

    const oldCreditAppliedToBill = await VendorCreditAppliedBill.query().findById(appliedCreditToBillId);
    ...
    const vendorCredit = await this.getVendorCreditOrThrowError(tenantId, oldCreditAppliedToBill.vendorCreditId);
    // Deletes vendor credit apply under unit-of-work envirement.
    return this.uow.withTransaction(tenantId, async (trx: Knex.Transaction) => {
      // Delete vendor credit applied to bill transaction.
      await VendorCreditAppliedBill.query(trx).findById(appliedCreditToBillId).delete();

      // Triggers `onVendorCreditApplyToInvoiceDeleted` event.
      await this.eventPublisher.emitAsync(events.vendorCredit.onApplyToInvoicesDeleted, {
        tenantId,
        vendorCredit,
