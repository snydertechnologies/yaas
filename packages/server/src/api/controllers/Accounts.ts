...
export default class AccountsController extends BaseController {
  ...
  private get createAccountDTOSchema() {
    return [
      check('name').exists().isLength({ min: 3, max: DATATYPES_LENGTH.STRING }).trim().escape(),
      check('code').optional({ nullable: true }).isLength({ min: 3, max: 6 }).trim().escape(),
      check('currency_code').optional(),
      check('account_type').exists().isLength({ min: 3, max: DATATYPES_LENGTH.STRING }).trim().escape(),
      check('description').optional({ nullable: true }).isLength({ max: DATATYPES_LENGTH.TEXT }).trim().escape(),
      check('parent_account_id').optional({ nullable: true }).isInt({ min: 0, max: DATATYPES_LENGTH.INT_10 }).toInt(),
    ];
  }
  ...
  private get accountsListSchema() {
    return [
      ...
      query('structure').optional().isString().isIn([IAccountsStructureType.Tree, IAccountsStructureType.Flat]),
    ];
  }

  get closingAccountSchema() {
    return [check('to_account_id').exists().isNumeric().toInt(), check('delete_after_closing').exists().isBoolean()];
  }
  ...
  async newAccount(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const account = await this.accountsApplication.createAccount(tenantId, accountDTO);
  ...
  async editAccount(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const account = await this.accountsApplication.editAccount(tenantId, accountId, accountDTO);
  ...
  async getAccount(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const account = await this.accountsApplication.getAccount(tenantId, accountId);
   ...
   * @param {Response}
   */
  public async getAccountsList(req: Request, res: Response, next: NextFunction) {
    const { tenantId } = req;
    ...
    try {
      const { accounts, filterMeta } = await this.accountsApplication.getAccounts(tenantId, filter);
  ...
  async accountTransactions(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const transactions = await this.accountsApplication.getAccountsTransactions(tenantId, transactionsFilter);
      return res.status(200).send({
  ...
  private catchServiceErrors(error, req: Request, res: Response, next: NextFunction) {
    if (error instanceof ServiceError) {
      ...
      if (error.errorType === 'account_type_not_allowed_to_changed') {
        return res.boom.badRequest('Not allowed to change account type of the account.', {
      ...
      if (error.errorType === 'account_has_associated_transactions') {
        return res.boom.badRequest('You could not delete account has associated transactions.', {
      ...
      if (error.errorType === 'predefined_accounts') {
        return res.boom.badRequest('Some of the given accounts are predefined.', {
          errors: [{ type: 'ACCOUNTS_PREDEFINED', code: 1100 }],
        });
      }
      if (error.errorType === 'close_account_and_to_account_not_same_type') {
        return res.boom.badRequest('The close account has different root type with to account.', {
          errors: [
            {
      ...
      if (error.errorType === 'ACCOUNT_TYPE_NOT_SUPPORTS_MULTI_CURRENCY') {
        return res.boom.badRequest('The given account type does not support multi-currency.', {
          errors: [{ type: 'ACCOUNT_TYPE_NOT_SUPPORTS_MULTI_CURRENCY', code: 1300 }],
        });
      }
      if (error.errorType === 'ACCOUNT_CURRENCY_NOT_SAME_PARENT_ACCOUNT') {
        return res.boom.badRequest('You could not add account has currency different on the parent account.', {
          errors: [{ type: 'ACCOUNT_CURRENCY_NOT_SAME_PARENT_ACCOUNT', code: 1400 }],
        });
      }
      if (error.errorType === 'PARENT_ACCOUNT_EXCEEDED_THE_DEPTH_LEVEL') {
        return res.boom.badRequest('The parent account exceeded the depth level of accounts chart.', {
          errors: [
            {
