...
export class ExpensesController extends BaseController {
  ...
  get expenseDTOSchema() {
    return [
      check('reference_no').optional({ nullable: true }).trim().escape().isLength({ max: DATATYPES_LENGTH.STRING }),
      check('payment_date').exists().isISO8601().toDate(),
      check('payment_account_id').exists().isInt({ max: DATATYPES_LENGTH.INT_10 }).toInt(),
      check('description').optional({ nullable: true }).isString().isLength({ max: DATATYPES_LENGTH.TEXT }),
      ...
      check('categories.*.index').exists().isInt({ max: DATATYPES_LENGTH.INT_10 }).toInt(),
      check('categories.*.expense_account_id').exists().isInt({ max: DATATYPES_LENGTH.INT_10 }).toInt(),
      check('categories.*.amount')
        .optional({ nullable: true })
        .isFloat({ max: DATATYPES_LENGTH.DECIMAL_13_3 }) // 13, 3
        .toFloat(),
      check('categories.*.description').optional().trim().escape().isLength({ max: DATATYPES_LENGTH.STRING }),
      check('categories.*.landed_cost').optional().isBoolean().toBoolean(),
      check('categories.*.project_id').optional({ nullable: true }).isInt({ max: DATATYPES_LENGTH.INT_10 }).toInt(),
    ];
  }
  ...
  async newExpense(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const expense = await this.expensesApplication.createExpense(tenantId, expenseDTO, user);
  ...
  async editExpense(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      await this.expensesApplication.editExpense(tenantId, expenseId, expenseDTO, user);
      return res.status(200).send({
        id: expenseId,
  ...
  async getExpensesList(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const { expenses, pagination, filterMeta } = await this.expensesApplication.getExpenses(tenantId, filter);
  ...
  async getExpense(req: Request, res: Response, next: NextFunction) {
    ...
    try {
      const expense = await this.expensesApplication.getExpense(tenantId, expenseId);
   ...
   */
  private catchServiceErrors(error: Error, req: Request, res: Response, next: NextFunction) {
    if (error instanceof ServiceError) {
      ...
      if (error.errorType === 'ENTRIES_ALLOCATED_COST_COULD_NOT_DELETED') {
        return res.status(400).send({
          errors: [{ type: 'ENTRIES_ALLOCATED_COST_COULD_NOT_DELETED', code: 1000 }],
        });
      }
