...
import { SalesTaxLiabilitySummaryApplication } from '@bigcapital/server/services/FinancialStatements/SalesTaxLiabilitySummary/SalesTaxLiabilitySummaryApplication';
...
export default class SalesTaxLiabilitySummary extends BaseFinancialReportController {
  ...
  public router() {
    ...
    router.get(
      '/',
      CheckPolicies(ReportsAction.READ_SALES_TAX_LIABILITY_SUMMARY, AbilitySubject.Report),
  ...
  private get validationSchema() {
    return [query('from_date').optional().isISO8601(), query('to_date').optional().isISO8601()];
  }
   ...
   */
  private async salesTaxLiabilitySummary(req: Request, res: Response, next: NextFunction) {
    const { tenantId } = req;
    ...
    try {
      ...
      if (acceptType === ACCEPT_TYPE.APPLICATION_JSON_TABLE) {
        const table = await this.salesTaxLiabilitySummaryApp.table(tenantId, filter);
        return res.status(200).send(table);
        // Retrieves the xlsx format.
      } else if (acceptType === ACCEPT_TYPE.APPLICATION_XLSX) {
        const buffer = await this.salesTaxLiabilitySummaryApp.xlsx(tenantId, filter);
        res.setHeader('Content-Disposition', 'attachment; filename=output.xlsx');
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        return res.send(buffer);
        // Retrieves the csv format.
      } else if (acceptType === ACCEPT_TYPE.APPLICATION_CSV) {
        const buffer = await this.salesTaxLiabilitySummaryApp.csv(tenantId, filter);
        ...
        const pdfContent = await this.salesTaxLiabilitySummaryApp.pdf(tenantId, filter);
        res.set({
        ...
        const sheet = await this.salesTaxLiabilitySummaryApp.sheet(tenantId, filter);
