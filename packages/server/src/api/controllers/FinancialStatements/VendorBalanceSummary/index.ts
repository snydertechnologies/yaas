...
import { VendorBalanceSummaryApplication } from '@bigcapital/server/services/FinancialStatements/VendorBalanceSummary/VendorBalanceSummaryApplication';
...
export default class VendorBalanceSummaryReportController extends BaseFinancialReportController {
  ...
  public router() {
    ...
    router.get(
      '/',
      CheckPolicies(ReportsAction.READ_VENDORS_SUMMARY_BALANCE, AbilitySubject.Report),
   ...
   */
  public async vendorBalanceSummary(req: Request, res: Response, next: NextFunction) {
    const { tenantId } = req;
    ...
    try {
      ...
      if (acceptType === ACCEPT_TYPE.APPLICATION_CSV) {
        ...
        const buffer = await this.vendorBalanceSummaryApp.xlsx(tenantId, filter);
        res.setHeader('Content-Disposition', 'attachment; filename=output.xlsx');
        res.setHeader('Content-Type', 'application/vnd.openxmlformats');
        return res.send(buffer);
        // Retrieves the json table format.
      } else if (acceptType === ACCEPT_TYPE.APPLICATION_JSON_TABLE) {
        const table = await this.vendorBalanceSummaryApp.table(tenantId, filter);
        return res.status(200).send(table);
        // Retrieves the pdf format.
      } else if (acceptType === ACCEPT_TYPE.APPLICATION_PDF) {
        const pdfContent = await this.vendorBalanceSummaryApp.pdf(tenantId, filter);
        res.set({
        ...
        const sheet = await this.vendorBalanceSummaryApp.sheet(tenantId, filter);
