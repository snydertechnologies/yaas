...
import { TransactionsByCustomerApplication } from '@bigcapital/server/services/FinancialStatements/TransactionsByCustomer/TransactionsByCustomersApplication';
...
export default class TransactionsByCustomersReportController extends BaseFinancialReportController {
  ...
  public router() {
    ...
    router.get(
      '/',
      CheckPolicies(ReportsAction.READ_CUSTOMERS_TRANSACTIONS, AbilitySubject.Report),
   ...
   */
  private async transactionsByCustomers(req: Request, res: Response, next: NextFunction) {
    const { tenantId } = req;
    ...
    try {
      ...
      if (ACCEPT_TYPE.APPLICATION_JSON_TABLE === acceptType) {
        const table = await this.transactionsByCustomersApp.table(tenantId, filter);
        ...
        const buffer = await this.transactionsByCustomersApp.xlsx(tenantId, filter);
        res.setHeader('Content-Disposition', 'attachment; filename=output.xlsx');
        res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
        return res.send(buffer);
        // Retrieve the json format.
      } else if (ACCEPT_TYPE.APPLICATION_PDF === acceptType) {
        const pdfContent = await this.transactionsByCustomersApp.pdf(tenantId, filter);
        res.set({
        ...
        const sheet = await this.transactionsByCustomersApp.sheet(tenantId, filter);
