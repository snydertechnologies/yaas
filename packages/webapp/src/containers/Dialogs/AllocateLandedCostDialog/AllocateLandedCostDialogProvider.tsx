...
function AllocateLandedCostDialogProvider({ billId, query, dialogName, ...props }) {
  ...
  const costTransaction = React.useMemo(
    () => (transactionId ? getCostTransactionById(transactionId, landedCostTransactions) : null),
    [transactionId, landedCostTransactions],
  );
  // Retrieve the cost transaction entry.
  const costTransactionEntry = React.useMemo(
    () => (costTransaction && transactionEntryId ? getTransactionEntryById(costTransaction, transactionEntryId) : null),
    [costTransaction, transactionEntryId],
  );
  // Retrieve entries of the given transaction id.
  const costTransactionEntries = React.useMemo(
    () => (transactionId ? getEntriesByTransactionId(landedCostTransactions, transactionId) : []),
  ...
  const unallocatedCostAmount = defaultTo(get(costTransactionEntry, 'unallocated_cost_amount'), 0);

  // Retrieve the unallocate cost amount of cost transaction.
  const formattedUnallocatedCostAmount = defaultTo(get(costTransactionEntry, 'formatted_unallocated_cost_amount'), 0);
...
const useAllocateLandedConstDialogContext = () => React.useContext(AllocateLandedCostDialogContext);
