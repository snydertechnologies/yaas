...
import { DetailItem, DetailsMenu, T } from '@bigcapital/webapp/components';
  ...
  return (
    <div className={clsx(Style.root_content)}>
      <DetailsMenu direction={'vertical'} className={clsx(Style.root_content_primary)}>
        <DetailItem name={'outstanding-payable'} label={<T id={'vendor.drawer.label.outstanding_payable'} />}>
          <h3 className="big-number">{vendor.formatted_balance}</h3>
        </DetailItem>

        <DetailItem label={<T id={'vendor.drawer.label.unused_credits'} />}>0</DetailItem>
      </DetailsMenu>

      <DetailsMenu direction={'horizantal'} minLabelSize={'175px'}>
        <DetailItem label={<T id={'vendor.drawer.label.vendor'} />} name={'name'}>
        ...
        />
        <DetailItem label={intl.get('email')} children={defaultTo(vendor.email, '--')} />
        <DetailItem label={<T id={'vendor.drawer.label.phone_number'} />}>
          <div>{vendor?.personal_phone} </div>
          <div>{vendor?.work_phone} </div>
        </DetailItem>

        <DetailItem label={<T id={'vendor.drawer.label.website'} />} children={defaultTo(vendor?.website, '--')} />
        <DetailItem
        ...
        />
        <DetailItem label={<T id={'vendor.drawer.label.currency'} />} children={vendor?.currency_code} />
        <DetailItem label={<T id={'vendor.drawer.label.note'} />} children={defaultTo(vendor?.note, '--')} />
      </DetailsMenu>
    </div>
  );
}
