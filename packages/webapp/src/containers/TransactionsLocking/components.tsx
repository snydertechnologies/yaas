...
export function TransactionsLockingList({ ...rest }) {
  ...
  return modules.map((module) => <TransactionsLockingItemModule module={module} {...rest} />);
}
  ...
  return (
    <TransactionLockingWrapp>
      <TransLockingInner>
        ...
        <TransLockingContent>
          <TransLockingItemTitle className={Classes.SKELETON}>XXXX</TransLockingItemTitle>
...
const useTransactionsLockingItemContext = () => React.useContext(TransactionsLockingItemContext);
      ...
      <If condition={isEnabled}>
        <TransLockWrap>
          ...
          <If condition={lockReason}>
            <TransLockingReason>
              {intl.formatHTMLMessage({ id: 'transactions_locking.lock_reason' }, { value: lockReason })}
            </TransLockingReason>
          </If>
        </TransLockWrap>
      </If>
      ...
      <If condition={isPartialUnlock}>
        <TransUnlockWrap>
          ...
          <If condition={unlockReason}>
            <TransLockingReason>
              {intl.formatHTMLMessage({ id: 'transactions_locking.unlock_reason' }, { value: unlockReason })}
            </TransLockingReason>
          </If>
        </TransUnlockWrap>
      </If>
...
function TransactionsLockingItemActions() {
  ...
  return (
    <TransLockingActions>
      <If condition={!isEnabled}>
        <Button small={true} minimal={true} intent={Intent.PRIMARY} onClick={handleLockClick}>
          <T id={'transactions_locking.lock'} />
        </Button>
      </If>

      <If condition={isEnabled}>
        <Button small={true} minimal={true} intent={Intent.PRIMARY} onClick={handleEditBtn}>
          <T id={'edit'} />
        </Button>
        <Divider />
        <Popover2
          content={
            <Menu>
              <MenuItem text={<T id={'transactions_locking.full_unlock'} />} onClick={handleUnlockFull} />

              <If condition={!isPartialUnlock}>
                <MenuItem text={<T id={'transactions_locking.paetial_unlock'} />} onClick={handleUnlockPartial} />
              </If>
              <If condition={isPartialUnlock}>
                <MenuItem
